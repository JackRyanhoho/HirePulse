...
// Mock function to simulate ai.models.generateContent for chat
export const generateChatResponse = (prompt: string): Promise<MockGenerateContentResponse> => {
        ...
        setTimeout(() => {
            ...
            if (foundSkills.length > 0) {
                filteredCandidates = CANDIDATE_DATA.filter(candidate => 
                    candidate.skills.some(skill => skill.name && foundSkills.includes(skill.name.toLowerCase()))
                );
            ...
            if (yearMatch) {
                ...
                filteredCandidates = filteredCandidates.filter(candidate => 
                    candidate.skills.some(skill => skill.years && skill.name && skill.years >= years && foundSkills.includes(skill.name.toLowerCase()))
                );
            }
